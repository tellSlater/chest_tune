 ...
 * The state of the chip's fuses should be (E:FF, H:FF, L:7A) when not locked.
 *
 *					  _________
 * PIN1 - N/C       _|	 O     |_		PIN8 - VCC
 * PIN2	- LED     	_|		   |_		PIN7 - Light senseor LDR
 * PIN3	- VGND      _|ATTiny13A|_		PIN6 - Melody
 * PIN4	- Ground	_|		   |_		PIN5 - Melody inverted
 *					 |_________|
 */ 
...
ISR (WDT_vect) {               //Wake from sleep and check darkness once every 8sec
	...
	_delay_ms(1);
	if (PINB & 1 << PINB2) {
		if (dark_times < 0xffff) dark_times++;
		PORTB &= ~(1 << PINB4);  // Disable sensor VCC
		if (dark_times > 450) {
			PORTB |= 1 << PINB4;       // Enable sensor VCC
			_delay_ms(1);
